...
ADMIN_PASSWORD = "YOUR_SECRET_CODE_HERE"
...
# Render automatically runs: gunicorn server:app
function checkLogin(){
    const code=document.getElementById('password-input').value;
    fetch(SERVER_URL+'/api/admin/check-pass',{
        method:'POST',
...
# -------------------------- NO app.run() — FIXED FOR RENDER --------------------------
# Render automatically uses: gunicorn server:app
function checkLogin(){
    const code=document.getElementById('password-input').value;
    fetch(SERVER_URL+'/api/admin/check-pass',{
        method:'POST',
...
'<td>' + i.status + '</td>' +
'<td>' + i.remaining + '</td>' +
'<td><button class=\"btn-danger\" onclick=\"deleteItem(\\'' + i.license + '\\')\">DELETE</button></td>';
    })
}
